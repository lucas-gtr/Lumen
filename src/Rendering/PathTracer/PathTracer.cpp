...
ColorRGB PathTracer::getDirectLighting(const PBR::BRDFInput& input, const linalg::Vec3d& light_dir,
                                       double& pdf) const {
...
ColorRGB PathTracer::traceRay(const Ray& ray, const ColorRGB& throughput, int depth, double previous_pdf_brdf, 
  ...
  const ColorRGB emitted_light = hit_info.material->getEmissive(hit_info.bary_coordinates) *
                                 hit_info.material->getEmissiveIntensity();
  ...
  const PBR::BRDFInput brdf_input = {
    hit_info.hit_point,
    incident,
    hit_info.normal,
  ...
  const double pdf_spec_light = Sampler::pdfHalfVectorGgx(reflection_prob, brdf_input.roughness, incident,
                                                          brdf_input.normal, h_light);
  ...
  // === Éclairage indirect (réflexion récursive) ===
  double pdf_brdf = 0.0;
  const linalg::Vec3d outgoing_dir = getOutgoingDirection(reflection_prob, brdf_input.roughness,
                                                          tbn, brdf_input.normal, incident, pdf_brdf);
  if (pdf_brdf <= 0.0) {
  ...
  const double pdf_spec_brdf = Sampler::pdfHalfVectorGgx(reflection_prob, brdf_input.roughness, incident,
                                                         brdf_input.normal, h_brdf);
  ...
  const ColorRGB indirect_lighting = traceRay(next_ray, new_throughput, depth + 1, pdf_brdf, pdf_list) * outgoing_radiance;
